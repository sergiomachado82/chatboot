...
/**
 * Processes an incoming guest message through the bot pipeline: classifies intent, accumulates entities, checks availability, and sends the appropriate response.
 * @param ctx - The bot context containing conversacionId, huespedId, huespedWaId, and the message text
 * @returns A promise that resolves when the message has been fully processed and the response sent
 */
export async function handleBotMessage(ctx: BotContext): Promise<void> {
  ...
  if (isPhotoRequest && classification.entities.habitacion) {
    try {
      ...
      logger.error(
        { err, depto: classification.entities.habitacion },
        'Error fetching/sending photos, falling through to text response',
      );
  ...
  if (shouldEscalateHumano || shouldEscalateQueja || shouldEscalateCancelacion || shouldEscalateModificacion) {
    ...
    await prisma.conversacion.update({
      ...
      data: { escaladaEn: new Date(), razonEscalacion: classification.intent },
    });
  ...
  if (photosFailed) {
    additionalContext +=
      'No tenemos fotos disponibles para enviar por este medio. NO menciones fotos, ni digas que se envian automaticamente, ni des excusas tecnicas. Simplemente describile el departamento con palabras y seguí la conversacion naturalmente.\n';
  }
    ...
    if (numPersonas > 0) {
      ...
      for (const c of complejos) {
        ...
        if (numPersonas <= c.capacidad) {
          capacityInfo.push(
            `- ${c.nombre}: APTO (max ${c.capacidad} pers/unidad, SOLO ${c.cantidadUnidades} unidad(es) — NUNCA ofrezcas mas de ${c.cantidadUnidades})`,
          );
        } else {
          capacityInfo.push(
            `- ${c.nombre}: NO APTO para ${numPersonas} personas (max ${c.capacidad} pers/unidad, SOLO ${c.cantidadUnidades} unidad(es))`,
          );
        }
      }
  ...
  if (missing.length > 0 && !isGenericQuery) {
    ...
    if (
      classification.intent === 'consulta_precio' &&
      (!mergedEntities.fecha_entrada || !mergedEntities.fecha_salida)
    ) {
  ...
  if (fecha_entrada && fecha_salida && !isGenericQuery) {
    try {
      const results = await checkAvailability(new Date(fecha_entrada), new Date(fecha_salida), habitacion);
      ...
      if (results.length > 0) {
        ...
        additionalContext += `Resultados de disponibilidad:\n${results
          .map(
            (r) =>
              `- Depto ${r.habitacion}: disponible, ${r.noches} noches, $${r.precioTotal} ARS total ($${r.precioPorNoche.join(', $')} ARS por noche)`,
          )
          .join('\n')}`;
            ...
            try {
              ...
              if (complejoData) {
                ...
                if (minNoches && noches < minNoches) {
                  minStayLines.push(
                    `- ${r.habitacion}: REQUIERE MINIMO ${minNoches} noches (${minSource}). El huesped solicita ${noches} noche(s) — NO CUMPLE.`,
                  );
                }
      ...
      additionalContext +=
        'No se pudo consultar la disponibilidad en este momento. Pide disculpas e indica que un agente verificara.';
    }
  }
  ...
  if (classification.intent === 'reservar' && mergedEntities.habitacion) {
    ...
    if (!hasBankData) {
      ...
      logger.warn(
        { habitacion: mergedEntities.habitacion, titular: deptoData?.titularCuenta },
        'Bank data has untrusted account holder — suppressing',
      );
  ...
  if (bankDataEscalate && botConfig.escalarSiPago) {
    ...
    await prisma.conversacion.update({
      where: { id: conversacionId },
      data: { escaladaEn: new Date(), razonEscalacion: 'datos_bancarios' },
    });
    await createMensaje({
      conversacionId,
      tipo: 'system',
  ...
  if (botConfig.autoPreReserva && classification.intent === 'reservar' && ctx.huespedId) {
    const allPresent =
      ...
      mergedEntities.dni;
    const prevHadAll =
      accumulated.habitacion &&
      accumulated.fecha_entrada &&
      accumulated.fecha_salida &&
      accumulated.num_personas &&
      accumulated.nombre_huesped &&
      accumulated.telefono &&
      accumulated.dni;
      ...
      try {
        ...
        await prisma.$transaction(
          async (tx) => {
            ...
            const avgTarifa =
              result.precioPorNoche.length > 0
            ...
            logger.info(
              { conversacionId, habitacion: mergedEntities.habitacion, precioTotal },
              'Auto-created reservation from bot conversation',
            );
          },
