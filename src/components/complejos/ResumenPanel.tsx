...
export default function ResumenPanel({ complejos }: ResumenPanelProps) {
  ...
  const preciosBaja = activos.flatMap((c) => c.tarifas.filter((t) => t.temporada === 'baja').map((t) => t.precioNoche));
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4 space-y-4">
      <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100">{t('complejos.resumenTitle')}</h3>

      <div className="grid grid-cols-3 gap-3">
        <div className="bg-blue-50 rounded-lg p-3 text-center">
          <p className="text-2xl font-bold text-blue-700">{activos.length}</p>
          <p className="text-xs text-blue-600">{t('complejos.resumenActive')}</p>
        </div>
        <div className="bg-purple-50 rounded-lg p-3 text-center">
          <p className="text-2xl font-bold text-purple-700">{totalUnidades}</p>
          <p className="text-xs text-purple-600">{t('complejos.resumenUnits')}</p>
        </div>
        <div className="bg-green-50 rounded-lg p-3 text-center">
          <p className="text-2xl font-bold text-green-700">{capacidadTotal}</p>
          <p className="text-xs text-green-600">{t('complejos.resumenCapacity')}</p>
        </div>
      </div>

      {preciosBaja.length > 0 && (
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('complejos.resumenPriceRange')}</p>
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
            {formatCurrency(precioMin)} - {formatCurrency(precioMax)}/noche
          </p>
        </div>
      )}
      ...
      <div>
        <h4 className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-2">
          {t('complejos.resumenDepartments')}
        </h4>
        <table className="w-full text-xs">
          <thead>
            <tr className="text-gray-400 dark:text-gray-500">
              <th className="text-left pb-1">{t('complejos.resumenColName')}</th>
              <th className="text-center pb-1">{t('complejos.resumenColUnits')}</th>
              <th className="text-center pb-1">{t('complejos.resumenColCapacity')}</th>
              <th className="text-right pb-1">{t('complejos.resumenColLowSeason')}</th>
            </tr>
          </thead>
          <tbody>
            {activos.map((c) => {
              ...
              return (
                <tr key={c.id} className="border-t border-gray-100 dark:border-gray-700">
                  ...
                  <td className="py-1 text-right text-gray-600 dark:text-gray-400">
                    {baja ? formatCurrency(baja.precioNoche) : '-'}
                  </td>
                </tr>
              );
            })}
