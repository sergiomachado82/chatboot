  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h4 className="text-sm font-semibold text-gray-700">Tarifas por noche (ARS)</h4>
        <div className="space-y-2">
          {TEMPORADAS.map((t) => (
            <div key={t.key} className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-500">Min noches</span>
                <input
                  type="number"
                  min={1}
      ...
      <div className="space-y-3 border-t pt-4">
        <h4 className="text-sm font-semibold text-gray-700">Tarifas Especiales (overrides por fechas)</h4>
        <p className="text-xs text-gray-500">Precios custom que sobreescriben la tarifa estacional para un rango de fechas. Se sincronizan al inventario automaticamente.</p>

        {/* Existing special rates list */}
        {tarifasEspeciales.length > 0 && (
          <div className="space-y-1">
            {tarifasEspeciales.map((te) => (
              <div key={te.id} className="flex items-center gap-3 bg-gray-50 rounded px-3 py-2 text-sm">
                <span className="text-gray-700">
                  {new Date(te.fechaInicio).toLocaleDateString('es-AR')} - {new Date(te.fechaFin).toLocaleDateString('es-AR')}
                </span>
                <span className="font-medium text-gray-900">${te.precioNoche.toLocaleString('es-AR')}/noche</span>
                {te.estadiaMinima && (
                  <span className="text-xs text-gray-500">min {te.estadiaMinima}n</span>
                )}
                {te.motivo && (
                  <span className="text-xs text-gray-400 italic">{te.motivo}</span>
                )}
                <button
        ...
        {tarifasEspeciales.length === 0 && (
          <p className="text-xs text-gray-400">Sin tarifas especiales</p>
        )}
        ...
        <div className="bg-blue-50 rounded p-3 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs text-gray-600 mb-0.5">Fecha inicio</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-xs text-gray-600 mb-0.5">Fecha fin</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-xs text-gray-600 mb-0.5">Precio/noche</label>
              <div className="flex items-center gap-1">
                <span className="text-sm text-gray-500">$</span>
                <input
                  type="number"
            ...
            <div>
              <label className="block text-xs text-gray-600 mb-0.5">Min noches (opc.)</label>
              <input
                type="number"
                min={1}
            ...
            <div className="col-span-2">
              <label className="block text-xs text-gray-600 mb-0.5">Motivo (opc.)</label>
              <input
                value={teForm.motivo}
      ...
      <div className="space-y-3 border-t pt-4">
        ...
        <p className="text-xs text-gray-500">Cerrar la disponibilidad para un rango de fechas (reparaciones, uso personal, etc). El bot no ofrecera el depto en esas fechas.</p>

        {bloqueos.length > 0 && (
          <div className="space-y-1">
            {bloqueos.map((b) => (
              <div key={b.id} className="flex items-center gap-3 bg-red-50 rounded px-3 py-2 text-sm">
                <span className="text-gray-700">
                ...
                {b.motivo && (
                  <span className="text-xs text-gray-500 italic">{b.motivo}</span>
                )}
                <button
        ...
        {bloqueos.length === 0 && (
          <p className="text-xs text-gray-400">Sin bloqueos activos</p>
        )}
        ...
        <div className="bg-red-50 rounded p-3 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {cantidadUnidades > 1 && (
              <div className="col-span-2">
                <label className="block text-xs text-gray-600 mb-0.5">Unidades a bloquear</label>
                <select
