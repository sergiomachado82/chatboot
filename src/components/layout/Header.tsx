...
const NAV_ITEMS: { path: string; icon: typeof MessageCircle; labelKey: string }[] = [
  { path: '/', icon: LayoutDashboard, labelKey: 'nav.dashboard' },
  { path: '/chat', icon: MessageCircle, labelKey: 'nav.chat' },
  { path: '/reservas', icon: Calendar, labelKey: 'nav.reservas' },
  { path: '/propiedades', icon: Building2, labelKey: 'nav.propiedades' },
  { path: '/emails', icon: Mail, labelKey: 'nav.emails' },
  { path: '/whatsapp', icon: Smartphone, labelKey: 'nav.whatsapp' },
  ...
  return (
    <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 md:px-6 py-2 flex items-center justify-between relative">
      <div className="flex items-center gap-3 md:gap-6">
        <h1 className="text-base md:text-lg font-bold text-gray-800 dark:text-gray-100 whitespace-nowrap">
          {t('app.title')}
        </h1>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <HealthIndicator />
        <span className="hidden sm:inline text-sm text-gray-500 dark:text-gray-400 truncate max-w-[120px]">
...
function HealthIndicator() {
  ...
  const dotColor = !health ? 'bg-gray-400' : health.status === 'ok' ? 'bg-green-500' : 'bg-yellow-500';
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={t('app.serviceStatusAria')}
        aria-expanded={showDetail}
      >
        <span className={`w-2 h-2 rounded-full ${dotColor}`} />
        <span className="hidden sm:inline">{t('app.serviceStatus')}</span>
      </button>
      ...
      {showDetail && health && (
        <div className="absolute right-0 top-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-3 z-50 min-w-48">
          <p className="text-xs font-semibold text-gray-700 dark:text-gray-200 mb-2">
            {t('app.serviceStatusLabel')}{' '}
            {health.status === 'ok' ? t('app.serviceStatusOk') : t('app.serviceStatusDegraded')}
          </p>
            ...
            {Object.entries(health.services).map(([key, svc]) => (
              <div key={key} className="flex items-center justify-between text-xs">
                ...
                <span className="flex items-center gap-1">
                  {svc.latencyMs != null && <span className="text-gray-400">{svc.latencyMs}ms</span>}
                  <span
...
function SocketIndicator() {
  ...
  const config = {
    connected: { icon: Wifi, color: 'text-green-500', label: t('app.realtimeConnected') },
    connecting: { icon: Wifi, color: 'text-amber-500 animate-pulse', label: t('app.realtimeConnecting') },
    disconnected: { icon: WifiOff, color: 'text-red-500', label: t('app.realtimeDisconnected') },
  }[status];
